...
@Directive({
  selector: 'input[appNumberFloatInput]',
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => NumberFloatInputDirective),
      multi: true
    }
  ],
  exportAs: 'appNumberFloatInput'
})
export class NumberFloatInputDirective implements ControlValueAccessor, OnChanges {
  ...
  @HostListener('input', ['$event']) public onInput(event: Event): void {
    ...
    const positionPoint = value.indexOf('.');
    if (value.length > 15) {
      console.log(positionPoint);
      if (positionPoint < 1) {
        this.element.nativeElement.value = value.substr(0, 15);
        event.preventDefault();
      } else {
        this.element.nativeElement.value = value.substr(0, positionPoint + this.decimals);
      }
    } else {
 ...
 //   Util.validateKeyPressInput(event, [8, 189 ]);
